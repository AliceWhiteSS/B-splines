function [SXY,DUSXY,DVSXY,DWSXY] = model4_3D(PX,PY,PZ,uspan,vspan,wspan,dersU,dersV,dersW,pU,pV,pW)

% arf_length = length(arf);
% beta_length = length(beta);
%
% PX = zeros(beta_length,arf_length);
% PY = zeros(beta_length,arf_length);
%iter  = 1;
...
for m = 1:pU+1
    ...
    for k = 1:pV+1
        ...
        for l = 1:pW+1
            ...
            temp11a = temp11a + Nw(l)* PX(uind,vind,wind+l);
