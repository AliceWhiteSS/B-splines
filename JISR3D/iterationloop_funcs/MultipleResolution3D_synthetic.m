function [M,F,phi,VF,VB] = MultipleResolution3D_synthetic(M,F,phi,param)
...
[X,Y,Z] = meshgrid(0.5:size(M,1)-0.5,0.5:size(M,2)-0.5,0.5:size(M,3)-0.5);
...
VF = zeros(size(M,1),size(M,2),size(M,3),3);
VB = zeros(size(M,1),size(M,2),size(M,3),3);
