while(abs(rs_final-rs_initial)>tol)
    ...
    temp_coef_xb = imfilter(VXb_new, Bsplinekernel); %Vx
    ...
    temp_coef_xb_inv = imfilter(VXb_new_inv, Bsplinekernel); %Vx
    ...
    Iplot = resize3Dmatrix(size(M00,1), size(M00,2), size(M00,3),phi/scale);
    Iplot = (Iplot-min(Iplot(:)))./(max(Iplot(:))-min(Iplot(:)));
    Mscale = (M-min(M(:)))./(max(M(:))-min(M(:)))*255;
    Fscale = (F_orig-min(F_orig(:)))./(max(F_orig(:))-min(F_orig(:)))*255;
    if(level==1)
        Mscale = zeros(181,181,217);
        Fscale = zeros(181,181,217);
        Mscale = M(11:191,11:191,4:220);
        Fscale = F_orig(11:191,11:191,4:220);
        Mscale = (Mscale-min(Mscale(:)))./(max(Mscale(:))-min(Mscale(:)))*255;
        Fscale = (Fscale-min(Fscale(:)))./(max(Fscale(:))-min(Fscale(:)))*255;
    end
    PlotImage(iterct,M_orig,F_orig,M);
    
    save('M_20_38.mat','M');
    save('phi_20_38.mat','phi');
    
    filename_vtk = sprintf('evolve%d.vtk',iterct);
    vtkwrite(filename_vtk, 'structured_grid',X,Y,Z,'scalars','Intensity',Iplot);
    
    Mscale(Mscale<0) = 0;
    Mscale(Mscale>255) = 255;
    Mscale = round(Mscale);
    Fscale(Fscale<0) = 0;
    Fscale(Fscale>255) = 255;
    Fscale = round(Fscale);
    computeDiceSimilarity(fid_out,iterct,Mscale,Fscale);
